...
def start():
    ...
    while True:
        ...
        center = (lpos + rpos) / 2
        error = -((165+484)//2 + 10 - center)
        error = (error+5)*10.0/170
        ...
        if time_now > 40:
            settime = 0.02
        if error<0:
            error_val = - math.pow(-error,2.5)
        else:
            error_val = math.pow(error,2.5)
        if time_now < 6:
            angle = PID(error_val,0.3,0.0001,0.01)
        else:
            angle = PID(error_val,0.5,0.0001,0.01)
        # 0.5 , 0.0001, 0.01
        angle = max(min(50,angle),-50)
        if (abs(angle-angle_ex) > 70):
            if time_now - one_time< settime:
                angle_ex = angle_ex
                drive(angle_ex,15)
                continue
            else:
        ...
        if (-6.5 < error < 6.5):
            if time_ex == 0:
                time_ex = time_now
            elif (time_now-time_ex>2.7):
                drive(angle,28)
            elif (time_now-time_ex>1.8):
                drive(angle,20)
            else:
                drive(angle,17)
        else:
